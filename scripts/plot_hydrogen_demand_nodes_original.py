...
DEFAULT_INPUT = (
    PROJECT_ROOT / "dev" / "360903_hydrogen_demand_nodes_all_iam_scenarios.csv"
)
...
REGION_LABELS = {
    "LAM": "Latin America",
    "OAS": "Other Asia",
    "REF": "Russia & former Soviet states",
...
def plot_demand_node_type(
    ...
    ax_hydrogen.set_ylabel(
        f"Hydrogen demand per {singular_label} \n(t H2/year)"
    )
    ax_nodes.set_ylabel(f"Number of {plural_label}")
    ax_hydrogen.grid(
        True, axis="both", linestyle=":", linewidth=0.8, alpha=0.6
    )
    ...
    fig.align_ylabels([ax_hydrogen, ax_nodes])
    fig.tight_layout()
