...
class _Summary:
    ...
    hydrogen_rows: list[dict[str, Any]] = field(default_factory=list)
    ...
    if events is None:
        ...
        for item in payload.get("events", ()) if isinstance(payload, Mapping) else ():
            try:
                ...
                continue
    for event in events:
        if event.computed_target_values.get("hydrogen report type"):
            summary.hydrogen_rows.append(
                {
                    "scenario": scenario_label,
                    "activity": event.activity.get("name"),
                    "location": event.activity.get("location"),
                    "status": event.status,
                    **event.computed_target_values,
                }
            )
...
def _write_workbook(
    *,
    ...
    if summary.hydrogen_rows:
        hydrogen_headers = list(
            dict.fromkeys(key for row in summary.hydrogen_rows for key in row)
        )
        _write_table_sheet(
            workbook, "Hydrogen", summary.hydrogen_rows, hydrogen_headers, 9
        )
