...
@pytest.mark.parametrize("supplier_location", ["RER", "RoW"])
def test_non_emptied_transport_market_checks_full_supplier_identity(supplier_location):
    ...
    activity = {
        ...
        "exchanges": [
            ...
            _transport_exchange(supplier, supplier_location),
        ],
    }
    validator = _transport_validator([activity])
    ...
    if supplier_location == "RER":
        assert len(validator.major_issues_log) == 1
        assert (
            validator.major_issues_log[0]["reason"] == "duplicate transport exchanges"
        )
    else:
        assert validator.major_issues_log == []
...
@pytest.mark.parametrize(
    "export_check", ["run_fast_export_session", "run_export_schema_checks"]
)
def test_export_rejects_orphan_hydrogen_sector_market(export_check):
    market = _hydrogen_sector_market("Transport")
    market.update(code="hydrogen-transport", database="target-db")
    market["exchanges"] = [
        {
            "name": market["name"],
            ...
            "type": "production",
            "amount": 1.0,
            "input": ("target-db", "hydrogen-transport"),
        }
    ]
    ...
    with pytest.raises(PremiseValidationError):
        getattr(validator, export_check)()
    assert any(
        issue["reason"] == "hydrogen sector market has no consumers"
        for issue in validator.major_issues_log
    )
